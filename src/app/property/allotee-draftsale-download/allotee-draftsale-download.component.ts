...
@Component({
  selector: 'app-allotee-draftsale-download',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './allotee-draftsale-download.component.html',
  styleUrl: './allotee-draftsale-download.component.scss'
})
export class AlloteeDraftsaleDownloadComponent {
  ...
  getAllApplicationData() {
    this.propertyService.getGenerateDraftSaleDeedByCustomerId(this.customerId, 0, "Generated").subscribe(
      (response: any) => {
  ...
  downloadDraftsaledeed(element: any) {
    let download = {
      "id": element.id,
      "draftSaleDeedDownloadStataus": "Yes"
    }
    this.propertyService.downloadDraftSaleDeed_allottee(download).subscribe(res => {
      if (res) {
        const pdfUrl =
