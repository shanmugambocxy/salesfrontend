...
@Component({
  selector: 'app-templatesdraftdeed',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './templatesdraftdeed.component.html',
  styleUrl: './templatesdraftdeed.component.scss'
})
export class TemplatesdraftdeedComponent {
      ...
      for (const fileType in this.uploadedFilePaths) {
        if (this.uploadedFilePaths[fileType]) {
          ...
          const data = {
            ...
            documentName: "Draft Deed", // Adjust this method to get the template name based on fileType
  ...
  async getAllTemplates() {
    try {
      const response = await this.propertyService.getTemplateBuDocument("Draft Deed").subscribe((res: any) => {
        if (res) {
          console.log('All templates:', response);
          this.responseObject = _.orderBy(res.responseObject, ['sno'], ['asc']);

          let fileName1 = this.responseObject.filter((x: any) => x.templateName == "10.1.Flat_DD");
          ...
          let fileName2 = this.responseObject.filter((x: any) => x.templateName == "10.2.House_DD");
          if (fileName2.length > 0) {
            this.type2 = fileName2[0].templateName
          } else {
            this.type2 = "";
          }
          let fileName3 = this.responseObject.filter((x: any) => x.templateName == "10.3.Plot_DD");
