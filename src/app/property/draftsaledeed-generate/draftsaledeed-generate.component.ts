...
@Component({
  selector: 'app-draftsaledeed-generate',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './draftsaledeed-generate.component.html',
  styleUrl: './draftsaledeed-generate.component.scss'
})
export class DraftsaledeedGenerateComponent {
  displayedColumns: string[] = ['sno', 'unitAccountNumber', 'schemeName', 'schemeType', 'unitNo', 'unittype', 'applicantName', 'demand', 'paid', 'balance', 'action'];
  ...
  getAllApplicationData() {
    this.propertyService.getGenerateDraftSaleDeed(0, "Ready").subscribe(
      (response: any) => {
  ...
  generateDraftSaleDeed(element: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        let generate = {
          ...
          "id": element.id,
          "draftSaleDeed": "Draft Sale Deed Path",
          "draftSaleDeedStatus": "Generated",
          "description": "Your Draft Sale Deed is Generated"
        }
        this.propertyService.updateDraftSaleDeedToGenerated(generate).subscribe(res => {
          if (res) {
            this.getAllApplicationData();
          }
        })
