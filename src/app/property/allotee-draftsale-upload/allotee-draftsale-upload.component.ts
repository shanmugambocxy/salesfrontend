...
@Component({
  selector: 'app-allotee-draftsale-upload',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './allotee-draftsale-upload.component.html',
  styleUrl: './allotee-draftsale-upload.component.scss'
})
export class AlloteeDraftsaleUploadComponent {
  displayedColumns: string[] = ['sno', 'unitAccountNumber', 'schemeName', 'schemeType', 'unitNo', 'unittype', 'applicantName', 'inprinciple', 'action'];
  ...
  getAllApplicationData() {
    this.propertyService.getNocuploadDataDraftsaledeedbyCustomerId(this.customerId, "Needed").subscribe(
      (response: any) => {
  ...
  uploadNoc(element: any) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
        title: 'Confirm Allotment',
        message: 'Are You Sure To upload Documents?'
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.propertyService.uploadNocDraftSaleDeedAllottee(uploadNoc).subscribe(res => {
          if (res) {
            this.getAllApplicationData();
          }
        })
