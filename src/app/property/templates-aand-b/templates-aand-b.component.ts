...
@Component({
  selector: 'app-templates-aand-b',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './templates-aand-b.component.html',
  styleUrl: './templates-aand-b.component.scss'
})
export class TemplatesAandBComponent {
      ...
      for (const fileType in this.uploadedFilePaths) {
        if (this.uploadedFilePaths[fileType]) {
          const data = {
            ...
            documentName: "AandB",  // Adjust this method to get the template name based on fileType
  ...
  getTemplateName(fileType: string): string {
    switch (fileType) {
      case 'Flat_AandB':
        return '7.1.OR_Flat_AandB';
      case 'House_AandB':
        return '7.2.OR_House_AandB';
      case 'Plot_AAandB':
        return '7.3.OR_Plot_AandB';
  ...
  async getAllTemplates() {
    try {
      const response = await this.propertyService.getTemplateBuDocument("AandB").subscribe((res: any) => {
        if (res) {
          console.log('All templates:', response);
          this.responseObject = _.orderBy(res.responseObject, ['sno'], ['asc']);

          let fileName1 = this.responseObject.filter((x: any) => x.templateName == "7.1.OR_Flat_AandB");
          ...
          let fileName2 = this.responseObject.filter((x: any) => x.templateName == "7.2.OR_House_AandB");
          if (fileName2.length > 0) {
            this.type2 = fileName2[0].templateName
          } else {
            this.type2 = "";
          }
          let fileName3 = this.responseObject.filter((x: any) => x.templateName == "7.3.OR_Plot_AandB");
