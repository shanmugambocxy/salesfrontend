...
@Component({
  selector: 'app-templatescancellation',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './templatescancellation.component.html',
  styleUrl: './templatescancellation.component.scss'
})
export class TemplatescancellationComponent {
      ...
      for (const fileType in this.uploadedFilePaths) {
        ...
        if (this.uploadedFilePaths[fileType]) {
          const data = {
            ...
            documentName: "Cancellation", // Adjust this method to get the template name based on fileType
  ...
  async getAllTemplates() {
    try {
      const response = await this.propertyService.getTemplateBuDocument("Cancellation").subscribe((res: any) => {
        if (res) {
