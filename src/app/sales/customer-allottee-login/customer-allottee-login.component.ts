...
@Component({
  selector: 'app-customer-allottee-login',
  standalone: true,
  imports: [SharedModule, CustomerHeaderComponent],
  templateUrl: './customer-allottee-login.component.html',
  styleUrl: './customer-allottee-login.component.scss'
})
export class CustomerAllotteeLoginComponent {
    ...
    if (this.form.valid && this.userInput === this.captcha) {
      ...
      this.authService.customerLogin(loginData).subscribe(
        async (response: any) => {
          ...
          if (response && response.responseObject.jwtCustomResponse.allottmentStatus == "Yes" && response.responseObject.jwtCustomResponse.message == "Authentication successful") {
